    ...
    for (i, bilin) in enumerate(bilins)
        for (odd, even) in [((1,1),(1,-1)), ((-1,-1),(-1,1))]
            if i <10
                ...
                as, bs = get_numquads(terms, un, nD; p1=bilin[1], p2=bilin[2], valp1=valp1, valp2=valp2)
                append!(aslist, as)
                append!(bslist, bs)
                tot = binomial(length(terms),nD-2)
                tott += tot
                ...
                #save_full(model, proc_rs, EoN_rs, rs_ws, 1; folder="test/n"*string(nD)*"/", bilin=bilin, valp1=valp1, valp2=valp2, ms=ms[k])
